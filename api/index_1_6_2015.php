...
$app->post('/getCleintInvitesLog', 'getCleintInvitesLog');
...
//            "online" => (!empty($onlineStatus[$key]) ? 1 : 0),
            ...
            $data_array = array("location" => $user_info, 'user_type' => "client");
...
function listing() {
    ...
    if (!empty($output->results[0]->geometry->location->lng)) {
        ...
        if (!empty($userIdList)) {
            ...
            $userinfoData = array();
            $userinfoData = $QuestionService->getQuestionData($user_ids, array('id', 'username', 'realname', 'birthdate', 'googlemap_location', 'field_f92bbdb57510b86ba6c506c487be3aa1', 'field_d3d1470339c8d689ab705fd19a509655'));
            $dob = date("Y/m/d", strtotime($userinfoData[54]["birthdate"]));
            $age = ageCalculate($dob);
            $avatar = $BOL_AvatarService_inst->getDataForUserAvatars($user_ids);
            $onlineStatus = $Userservice->findOnlineStatusForUserList($user_ids);
            $user_info = array();
...
function getProfiledetails() {
    ...
    if (!empty($user_ids)) {
        ...
        if (!empty($user)) {
            ...
            if ($account == "808aa8ca354f51c5a3868dad5298cd72") {
                $type = "preferences";
                $sex = "client";
            } else {
            ...
            $userinfoData = $QuestionService->getQuestionData($user_ids, array('id', 'username', 'birthdate', 'email', 'googlemap_location', 'field_f92bbdb57510b86ba6c506c487be3aa1', 'field_e9795a9d3be797f58aab38b1033265a4', HAMMU_DB_MOBILE_NUMBER_KEY));
...
//    die;
    ...
    foreach ($datas as $key => $data) {
        ...
        $message = $language->text('hammu', $data["client_log"], array('user' => $inviteeUsername, "date" => $date, "time" => $time));
...
//    die;
    ...
    foreach ($datas as $key => $data) {
        ...
        $date = date("Y/m/d H:i", $data["timestamp"]);
        $time = date("H:i", $data["timestamp"]);
        $message = $language->text('hammu', $data["escort_log"], array('user' => $inviteeUsername, "date" => $date, "time" => $time));
...
//$user_infos;
    ...
    if (!empty($datas)) {
        ...
        $return_data = array("response_status" => "0", "data" => "There is no notification");
        $app->response->setBody(json_encode($return_data));
    }
}
