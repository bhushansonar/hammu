...
//Client = 3f50168064b9153e37ba1de896cd64ca
...
function signup() {
    ...
    $field_391797ad0e06d17d5b5cec0e48def7c2 = $app->request()->params('field_391797ad0e06d17d5b5cec0e48def7c2'); //mobile
...
//            exit;
            if (!empty($output->results[0]->formatted_address)) {
                $data['googlemap_location'] = $output->results[0]->formatted_address;
                $data['googlemap_location'] = $output->results[0]->geometry->location->lat;
                $data['googlemap_location'] = $output->results[0]->geometry->location->lng;
                $data['googlemap_location'] = $output->results[0]->geometry->bounds->northeast->lat;
                $data['googlemap_location'] = $output->results[0]->geometry->bounds->northeast->lng;
                $data['googlemap_location'] = $output->results[0]->geometry->bounds->southwest->lat;
                $data['googlemap_location'] = $output->results[0]->geometry->bounds->southwest->lng;
                $data['googlemap_location'] = json_encode($output->results[0]);
            }
        }
        echo "<pre>";
        print_r($data);
...
//$accountype = ($sex == "female") ? "8cc28eaddb382d7c6a94aeea9ec029fb" : "808aa8ca354f51c5a3868dad5298cd72"; //(int) $data["sex"]; //(1 = Female, 2 = Male)
        if ($accounttype == "3f50168064b9153e37ba1de896cd64ca") {
...
//                        WHERE `flag` = '0'";
    $sql = "SELECT COUNT(*) as notification_count FROM " . OW_DB_PREFIX . "invites_log
                        WHERE inviterId={$inviterId} AND inviteeId={$inviteeId}";
