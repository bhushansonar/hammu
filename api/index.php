...
$app->post('/getAllLanguages', 'getAllLanguages');
$app->post('/isFavoriteOrNot', 'isFavoriteOrNot');
...
//Geo location settings on address
            ...
            if (!empty($data["googlemap_location"])) {
                ...
                if (!empty($output->results[0]->formatted_address)) {
                    ...
                    $data_location['googlemap_location']['address'] = isset($output->results[0]->formatted_address) ? $output->results[0]->formatted_address : "";
                    $data_location['googlemap_location']['latitude'] = isset($output->results[0]->geometry->location->lat) ? $output->results[0]->geometry->location->lat : "";
                    $data_location['googlemap_location']['longitude'] = isset($output->results[0]->geometry->location->lng) ? $output->results[0]->geometry->location->lng : "";
                    $data_location['googlemap_location']['northEastLat'] = isset($output->results[0]->geometry->bounds->northeast->lat) ? $output->results[0]->geometry->bounds->northeast->lat : $output->results[0]->geometry->location->lat;
                    $data_location['googlemap_location']['northEastLng'] = isset($output->results[0]->geometry->bounds->northeast->lng) ? $output->results[0]->geometry->bounds->northeast->lng : $output->results[0]->geometry->location->lng;
                    $data_location['googlemap_location']['southWestLat'] = isset($output->results[0]->geometry->bounds->southwest->lat) ? $output->results[0]->geometry->bounds->southwest->lat : $output->results[0]->geometry->location->lat;
                    $data_location['googlemap_location']['southWestLng'] = isset($output->results[0]->geometry->bounds->southwest->lng) ? $output->results[0]->geometry->bounds->southwest->lng : $output->results[0]->geometry->location->lng;
                ...
                $message = $language->text("hammu", "signup_error_location"); //"Please Provide Proper Date!";
                $return_data = array("response_status" => "0", "response_message" => $message);
                $app->response->setBody(json_encode($return_data));
            }
        }
...
function advance_search() {
    ...
    $username = $app->request()->params('realname');
    ...
    $data_arr = array("form_name" => "MainSearchForm", "sex" => $sex, "match_sex" => $match_sex, "username" => $username, "with_photo" => $with_photo, "online" => $online, "googlemap_location" => $data_location, "birthdate" => array("from" => $age_min, "to" => $age_max), HAMMU_DB_SERVICES_KEY => $serach_services, HAMMU_DB_IM_USING_HAMMU_AS_KEY => "2", "available" => $available, "MainSearchFormSubmit" => "Search");
...
//        exit;
        ...
        if (!empty($output->results[0]->geometry->location->lng)) {
            ...
            $search_location_lat_long = array('latitude' => "", 'longitude' => "");
        }
...
//        die;
        ...
        foreach ($userinfoData as $key => $user) {
            ...
            if (array_key_exists(HAMMU_AVAILABLE_AT, $user)) {
                ...
                $HAMMU_AVAILABLE_AT_VALUE = $availabel_at_all[0]["userSelectedValue"];   //$user[HAMMU_AVAILABLE_AT];
                $HAMMU_AVAILABLE_AT_label = $availabel_at_all[0]["userSelectedLabel"]; //$user[HAMMU_AVAILABLE_AT];
            } else {
            ...
            $user_data[] = array(
                ...
                HAMMU_AVAILABLE_AT => $HAMMU_AVAILABLE_AT_VALUE,
                HAMMU_AVAILABLE_AT . "_label" => $HAMMU_AVAILABLE_AT_label,
                HAMMU_SECRET_FANTASY => $HAMMU_SECRET_FANTASY,
                HAMMU_LANGUAGE => $HAMMU_LANGUAGE,
                HAMMU_COUNTRY => $HAMMU_COUNTRY,
                "language_lable" => $language_lable,
...
function getAllLanguages() {
    ...
    //Lang Call end
    $filed_array = array(HAMMU_LANG);
    $all_language = getallquestionsForservices(array("fields" => $filed_array));
    $app->response->setBody(json_encode($all_language));
}
                ...
                foreach ($all_services_preferences as $key => $user_services_preferences) {
                    foreach ($userinfoData as $key => $user) {
                        ...
                        if (!empty($user[HAMMU_AVAILABLE_AT])) {
                            $availabel_at_all = getallquestions(array("user_id" => $user_id, "fields" => array(HAMMU_AVAILABLE_AT)));
...
//                            die;
                        ...
                        $user_data = array(
                            ...
                            "language_lable" => $all_languages[0]['userSelectedLabel'],
                            "language_value" => $all_languages[0]['userSelectedValue'],
                        );
                        //
                    }
                }
                ...
                $return_data = array("response_status" => "1", "response_message" => $message, "data" => $user_data);
                header("Keep-Alive: timeout=31, max=200", TRUE);
                $app->response->setBody(json_encode($return_data));
            } else {
...
function setProfiledetails() {
    ...
    if ($birthday_time > $today_date) {
        ...
        if ($output->status == "OK") {
            ...
            $data['googlemap_location'] = array(
                'address' => isset($output->results[0]->formatted_address) ? $output->results[0]->formatted_address : "",
                ...
                'northEastLat' => isset($output->results[0]->geometry->bounds->northeast->lat) ? $output->results[0]->geometry->bounds->northeast->lat : $output->results[0]->geometry->location->lat,
                'northEastLng' => isset($output->results[0]->geometry->bounds->northeast->lng) ? $output->results[0]->geometry->bounds->northeast->lng : $output->results[0]->geometry->location->lng,
                'southWestLat' => isset($output->results[0]->geometry->bounds->southwest->lat) ? $output->results[0]->geometry->bounds->southwest->lat : $output->results[0]->geometry->location->lat,
                'southWestLng' => isset($output->results[0]->geometry->bounds->southwest->lng) ? $output->results[0]->geometry->bounds->southwest->lng : $output->results[0]->geometry->location->lng,
                'json' => json_encode($output->results[0]),
            );
//            $data['googlemap_location']['address'] = isset($output->results[0]->formatted_address) ? $output->results[0]->formatted_address : "";
//            $data['googlemap_location']['latitude'] = isset($output->results[0]->geometry->location->lat) ? $output->results[0]->geometry->location->lat : "";
//            $data['googlemap_location']['longitude'] = isset($output->results[0]->geometry->location->lng) ? $output->results[0]->geometry->location->lng : "";
//            $data['googlemap_location']['northEastLat'] = isset($output->results[0]->geometry->bounds->northeast->lat) ? $output->results[0]->geometry->bounds->northeast->lat : $output->results[0]->geometry->location->lat;
//            $data['googlemap_location']['northEastLng'] = isset($output->results[0]->geometry->bounds->northeast->lng) ? $output->results[0]->geometry->bounds->northeast->lng : $output->results[0]->geometry->location->lng;
//            $data['googlemap_location']['southWestLat'] = isset($output->results[0]->geometry->bounds->southwest->lat) ? $output->results[0]->geometry->bounds->southwest->lat : $output->results[0]->geometry->location->lat;
//            $data['googlemap_location']['southWestLng'] = isset($output->results[0]->geometry->bounds->southwest->lng) ? $output->results[0]->geometry->bounds->southwest->lng : $output->results[0]->geometry->location->lng;
...
function getClientInvitesLog() {
    ...
    foreach ($datas as $key => $data) {
        ...
        if (!empty($json_string)) {
            ...
            if (!empty($meeting_data_arr['date'])) {
...
function getEscortInvitesLog() {
    ...
    foreach ($datas as $key => $data) {
        ...
        if ($data["action"] == "escort_propose_date_decline") {
            $inviteeUsername = $Userservice->getUserName($data["inviteeId"]);
            $invitee_name = $Userservice->getUserName($data["inviterId"]) . "'s";
        } else {
            $inviteeUsername = $Userservice->getUserName($data["inviterId"]);
            $invitee_name = "";
        }
        $message = $language->text('hammu', $data["escort_log"], array('user' => $inviteeUsername, "date" => $date, "time" => $time, "data" => $data_content, "location" => $location, "instructions" => $instruction, "meeting_date" => $meeting_date, "meeting_time" => $meeting_time, "invitee_name" => $invitee_name));
        $return_array[] = array(
            "message" => $message,
            "user_id" => $id
        );
...
function escortCountNotification() {
    ...
    //Lang Call end
    $inviterId = $app->request()->params("inviterId");
    $sql = "SELECT COUNT(*) as notification_count FROM " . OW_DB_PREFIX . "invites_log WHERE inviteeId={$inviterId} AND action IN ('invitation_sent','client_agree','client_decline','buy_rose','reject','escort_propose_date_decline') AND `count` = '0' AND `noti_remove`='0'";
...
function clientCountNotification() {
    ...
    //Lang Call end
    $inviterId = $app->request()->params("inviterId");
    $sql = "SELECT COUNT(*) as notification_count FROM " . OW_DB_PREFIX . "invites_log WHERE inviterId = {$inviterId} AND action IN ('invitation_accept','escort_asks','invitation_sent_fail','escort_decline','client_re_arrange','propose_date_accept','meet_location','buy_rose_client','escort_propose_date_decline') AND `count` = '0' AND `noti_remove`='0'";
...
//        exit;
        ...
        foreach ($datas as $key => $data) {
            $flag = NULL;
            if ($data["action"] == "escort_decline" || $data["action"] == "escort_propose_date_decline") {
                $inviteeUsername = $Userservice->getUserName($data["inviteeId"]);
                $invitee_name = $Userservice->getUserName($data["inviterId"]) . "'s";
            } else {
                $inviteeUsername = $Userservice->getUserName($data["inviterId"]);
            ...
            //echo $data_content;
            $message = $language->text('hammu', $data["action"] . "_noti", array('user' => $inviteeUsername, "date" => $date, "time" => $time, "data" => $data_content, "invitee_name" => $invitee_name));
//            echo $message;
//            die;
            $flag = "none";
            ...
            $return_array[$data["invitesId"]] = array(
                "invitesId" => $data["invitesId"],
                "message" => $message,
                "id" => $data["id"],
                "data" => $data["data"],
                "flag" => $flag,
...
//        exit;
        ...
        //$app->response->header('Keep-Alive', 'timeout=30,max=100');
        //$app->response()->header('60', '200');
        $app->response->setBody(json_encode($return_data));
    } else {
        $message = $language->text("hammu", "client_notification_list_fail"); //"You have No Notification"
        $return_data = array("response_status" => "0", "response_message" => $message);
        //header("Keep-Alive: timeout=60, max=100");
        //$app->response->headers->set('Keep-Alive', 'timeout=30, max=100');
        //$app->response()->header('60', '200');
        $app->response->setBody(json_encode($return_data));
    }
}
...
function getWhatIsHAMMU() {
    ...
    $return_data = array("response_status" => "1", "data" => $langValue);
    $app->response->setBody(json_encode($return_data));
}
...
function contactUs() {
    ...
    foreach ($contacts as $key => $contact) {
        ...
        $mail = $getMailer->createMail();
        $mail->addRecipientEmail($contact->email);
...
//    $mail = $getMailer->createMail();
//    $mail->addRecipientEmail($contactEmails[3]['email']);
//    $mail->setSender($email);
//    $mail->setSenderSuffix(false);
//    $mail->setSubject($subject);
//    $mail->setTextContent($message);
...
function isFavoriteOrNot() {
    ...
    $is_fav_or_not = $OWgetDbo->queryForList($sql);
    if (!empty($is_fav_or_not)) {
        //$message = $language->text("hammu", "delete_noti_success");
        $message = "favorite";
        $return_data = array('response_status' => "1", 'response_message' => $message, 'is_favorite' => "1");
        $app->response->setBody(json_encode($return_data));
    } else {
        //$message = $language->text("hammu", "delete_noti_success");
        $message = "Unfavorite";
        $return_data = array('response_status' => "0", 'response_message' => $message, 'is_favorite' => "0");
        $app->response->setBody(json_encode($return_data));
    }
}
...
function getUserInfo($userId) {
    ...
    //hardik edit
    $userinfoData = $QuestionService->getQuestionData(array($userId), array('id', 'username', 'realname', 'birthdate', 'email', 'googlemap_location', HAMMU_DB_SERVICES_KEY, HAMMU_DB_MOBILE_NUMBER_KEY, HAMMU_DB_PRICE_KEY, HAMMU_HOTEL_SERVICE, HAMMU_HOME_VISIT, HAMMU_AVAILABLE_AT, HAMMU_SECRET_FANTASY, HAMMU_LANGUAGE, HAMMU_COUNTRY, HAMMU_LANG));
    //print_r($userinfoData);
...
//                    exit;
                    ...
                    $user_data = array(
                        ...
                        "age" => "$age",
                        "profile_picture" => $avatar[$key]["src"],
                        "prices" => !empty($user[HAMMU_DB_PRICE_KEY]) ? $user[HAMMU_DB_PRICE_KEY] : "",
...
function acceptInvites() {
    ...
    $rarray = $HAMMU_BOL_Service->accept($inviterId, $inviteeId, $id, $invitesId);
    if (count($rarray) > 0) {
...
function reArrageInvites() {
    ...
    if (!empty($output->results[0]->geometry->location->lng)) {
        ...
        $rarray = $HAMMU_BOL_Service->invite_re_arrange($inviterId, $inviteeId, $id, $post_timestamp, $addrr, $invitesId);
        if (count($rarray) > 0) {
...
function acceptProposeDate() {
    ...
    if (!empty($output->results[0]->geometry->location->lng)) {
        ...
        $addrr = json_encode($store_location_details);
        $rarray = $HAMMU_BOL_Service->propose_date_accept($inviterId, $inviteeId, $id, $addrr, $invitesId);
        if (count($rarray) > 0) {
...
function reject() {
    ...
    $inviteeId = $app->request()->params("inviteeId");
    $id = $app->request()->params("id");
    $meeting_date = $app->request()->params("meeting_date");

    $rarray = $HAMMU_BOL_Service->reject($inviterId, $inviteeId, $id, $meeting_date);

    if (count($rarray) > 0) {
        $message = $language->text("hammu", "rejection_success"); // "Propose Date accept!"
        $return_data = array("response_status" => "1", "response_message" => $message);
        $app->response->setBody(json_encode($return_data));
    } else {
        $message = $language->text("hammu", "rejection_fail"); //"Propose Date already accept!"
        $return_data = array("response_status" => "0", "response_message" => $message);
        $app->response->setBody(json_encode($return_data));
    }
}
...
function paypal() {
    ...
    if ($state == 'approved') {
        ...
        $rarray = $HAMMU_BOL_Service->confirm_invite($inviterId, $inviteeId, $id, $addrr, $invitesId);
        $user_details = getUserInfo($inviteeId);
        if (count($rarray) > 0) {
            $message = $language->text("hammu", "paypal_success", array("user" => $user_details["user_name"])); //"THANK YOUR FOR BUYING {$user_details['user_name']} A ROSE {$user_details['user_name']} NOW PROVIDES YOU HER CONTACT DETAILS"
            $return_data = array("response_status" => "1", "response_message" => $message);
            $app->response->setBody(json_encode($return_data));
        } else {
