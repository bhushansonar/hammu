...
function reject() {
    ...
    $rarray = $HAMMU_BOL_Service->reject($inviterId, $inviteeId, $id);
    if (count($rarray) > 0) {
...
function clientCountNotification() {
    ...
    //Lang Call end
    $inviterId = $app->request()->params("inviterId");
    $sql = "SELECT COUNT(*) as notification_count FROM " . OW_DB_PREFIX . "invites_log WHERE inviterId = {$inviterId} AND action IN ('invitation_accept','escort_asks','invitation_sent_fail','escort_decline','client_re_arrange','propose_date_accept','meet_location','buy_rose_client','reject','escort_propose_date_decline') AND `count` = '0'";
