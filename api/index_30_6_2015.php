...
function login() {
    ...
    if ($email_exits != '1') {
        ...
        if (!$result->isValid()) {
            ...
            if ($type_check == '1') {
                $sex = $language->text("hammu", "login_client"); //"client";
            } else {
                $sex = $language->text("hammu", "login_escort"); //"escort";
            }
...
//end bhushan changes
        ...
        $userinfoData = array();
        $userinfoData = $QuestionService->getQuestionData($user_ids, array('id', 'username', 'realname', 'birthdate', 'googlemap_location', HAMMU_DB_SERVICES_KEY, HAMMU_DB_PRICE_KEY, HAMMU_HOTEL_SERVICE, HAMMU_HOME_VISIT, HAMMU_SECRET_FANTASY));
...
function setProfiledetails() {
    ...
    if ($birthday_time > $today_date) {
        ...
        $filed_array = array('realname', 'birthdate', 'email', 'googlemap_location', 'preference_or_services', HAMMU_DB_MOBILE_NUMBER_KEY, HAMMU_DB_PRICE_KEY, HAMMU_DB_PAYMENT_TYPE_KEY, HAMMU_HOTEL_SERVICE, HAMMU_HOME_VISIT, HAMMU_SECRET_FANTASY);
        $data = array();
