...
function login() {
    ...
    if ($email_exits != '1') {
        ...
        if (!$result->isValid()) {
            ...
            if ($account_type == "3f50168064b9153e37ba1de896cd64ca") {
                $sex = "client";
            } else {
                $sex = "escort";
            }
...
//Geo location settings on address
        ...
        if (!empty($data["googlemap_location"])) {
            ...
            if (!empty($output->results[0]->formatted_address)) {
                ...
                $data_location['field_391797ad0e06d17d5b5cec0e48def7c2'] = $data["field_391797ad0e06d17d5b5cec0e48def7c2"];
                $data_location['sex'] = $data["sex"];
                $data_location['match_sex'] = $data["match_sex"];
                $data_location['realname'] = $data["realname"];
                $data_location[$type_value] = $preVal;
            }
        }
