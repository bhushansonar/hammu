...
function getEscortInvitesLog() {
    ...
    foreach ($datas as $key => $data) {
        ...
        $message = $language->text('hammu', $data["escort_log"], array('user' => $inviteeUsername, "date" => $date, "time" => $time, "data" => $data_content, "location" => $location, "instructions" => $instruction, "meeting_date" => $meeting_date, "meeting_time" => $meeting_time));
        $return_array[] = array(
            "message" => $message,
            "user_id" => $id
        );
