...
$app->post('/quick_serch', 'quick_serch');
...
//            $check_exist_value = $SKAPI_BOL_Service_inst->findValueExistOrNot($user_id);
//            $user_skapi_id = $check_exist_value[0]->id;

            $user_details = $SKAPI_BOL_Service_inst->createUserDetails($user_id, $deviceId, $deviceType);
...
//            "online" => (!empty($onlineStatus[$key]) ? 1 : 0),
            ...
            $data_array = array("location" => $user_info, 'user_type' => "gentlemen");
...
function searchResult($params, $listId) {
    ...
    $filed_array = array("realname", "sex", "googlemap_location", "relationship",
        "6341ee69d879c97cb0c47507a557372c", "match_sex", "a69350b819c8dc1d926cba6b58451f5f",
        "31d5d3d7ff420e340cfe3f17c534eba3", "23036b67a077fe7784c7cf4691a96fe3", "36e39f917cbcf4fb10aa753186ced60d",
        "aboutme", "2b29f145d9d1128a3f57d3af6798fc85", "18af0f3559b872c87467a5a89edf03e",
        "match_age", "birthdate", "6ed883b3412cab27014a0b68ea32f269", "field_b63db9264225396873b5b6e9a02dbc66",
        "field_8669de45f87c662844ac8cb7bf1803a4", "email", "username", "field_e9795a9d3be797f58aab38b1033265a4", "avatar"
    );
    //echo "<pre>";
...
function setProfiledetails() {
    ...
    $filed_array = array("realname", "sex", "googlemap_location", "relationship",
        ...
        "aboutme", "2b29f145d9d1128a3f57d3af6798fc85", "18af0f3559b872c87467a5a89edf03e",
        "match_age", "birthdate", "6ed883b3412cab27014a0b68ea32f269", "field_b63db9264225396873b5b6e9a02dbc66",
        "field_8669de45f87c662844ac8cb7bf1803a4", "email", "us ername", "field_e9795a9d3be797f58aab38b1033265a4", "avatar"
    );
    $data = array();
    foreach
    ($filed_array as $fileds) {
...
function renderQuestion($userId, $questionName, $label = false) {
    ...
    $questionData = $QuestionService->getQuestionData(array(
        $userId), array($questionName));
    ...
    switch ($question->presentation) {
        ...
        case $QUESTION_PRESENTATION_AGE:
            $date = UTIL_DateTime::parseDate($questionData[$userId][$question->name], $MYSQL_DATETIME_DATE_FORMAT);
            $questionData[$userId][$question->name] = date("Y/m/d", strtotime($date['year'] . "/" . $date['month'] . "/" . $date['day'])); //UTIL_DateTime::getAge($date['year'], $date['month'], $date['day']) . " " . $language->text('base', 'questions_age_year_old');
