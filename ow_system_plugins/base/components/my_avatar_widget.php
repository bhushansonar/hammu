...
//        echo '</pre>';
    ...
    public static function getStandardSettingValueList() {
        //hardik chnages for sidebar title
        // echo "call->" . OW::getLanguage()->text('base', 'my_avatar_widget');
        $userId = OW::getUser()->getId();
        if ($userId) {
            $username = OW::getUser()->getUserObject()->getUsername();
        } else {
            $username = OW::getLanguage()->text('base', 'my_avatar_widget');
        }
        return array(
            self::SETTING_AVALIABLE_SECTIONS => array(BOL_ComponentService::SECTION_SIDEBAR),
            self::SETTING_TITLE => $username, //OW::getLanguage()->text('base', 'my_avatar_widget'),
