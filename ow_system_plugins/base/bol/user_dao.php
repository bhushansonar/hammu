...
//                    echo '</pre>';
                    ...
                    if ((!empty($data['join']) && $question->name != "match_sex") || !empty($data['where'])) {
                        //echo "<br>if name->" . $question->name;
                        // echo "if call";
                        if ($this->dbo->escapeString($question->name) != "sex") {
                            $innerJoin .= $event->getJoin();
                        }
                        $where .= ' AND ' . $event->getWhere();
                    } else {
                        //  echo "<br>else name->" . $question->name;
                        // echo "<br/>name->" . $question->name . "===" . $questionValues[$question->name];
                        // die
                        // echo "else call";
                        $questionString = $this->getQuestionWhereString($question, $questionValues[$question->name], $prefix . $counter);
                        if (!empty($questionString)) {
                            if ($this->dbo->escapeString($question->name) == "sex") {
                                $tablname = "match_sex";
                            } else if ($this->dbo->escapeString($question->name) == "match_sex") {
                                $tablname = "sex";
                            } else {
                                $tablname = $this->dbo->escapeString($question->name);
                            }
                            // echo "<br/>str->" . $str .= $questionString;
                            if ($this->dbo->escapeString($question->name) != "sex") {
                                $innerJoin .= " INNER JOIN `" . BOL_QuestionDataDao::getInstance()->getTableName() . "` `" . $prefix . $counter . "`
                                ON ( `user`.`id` = `" . $prefix . $counter . "`.`userId` AND `" . $prefix . $counter . "`.`questionName` = '" . $tablname . "' AND " . $questionString . " ) ";
                            }
                            //die;
                        }
                    }
