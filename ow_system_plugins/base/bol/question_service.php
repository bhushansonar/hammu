...
class BOL_QuestionService {
    ...
    public function getFormElementByQuestionDto(BOL_Question $question, array $values, $type = 'join') {
        ...
        if (empty($presentation)) {
            return $class;
        } else if (!empty($this->presentations[$presentation]) && ( $question->type == $this->presentations[$presentation] || in_array($question->type, array(self::QUESTION_VALUE_TYPE_SELECT, self::QUESTION_VALUE_TYPE_MULTISELECT)) && in_array($this->presentations[$presentation], array(self::QUESTION_VALUE_TYPE_SELECT, self::QUESTION_VALUE_TYPE_MULTISELECT)) )) {
            // ignore
        } else {
            return $class;
        }
...
//                        }
                        ...
                        if (!empty($date)) {
                            ...
                            $value = date("Y-m-d", strtotime($date)); //$date[UTIL_DateTime::PARSE_DATE_YEAR] . '-' . $date[UTIL_DateTime::PARSE_DATE_MONTH] . '-' . $date[UTIL_DateTime::PARSE_DATE_DAY];
    ...
    public function getRequiredQuestionsForNewAccountType() {
        ...
        foreach ($questions as $question) {
            ...
            if ($question->base == 1 || in_array($question->name, array('birthdate', 'realname', 'match_sex', 'sex', 'joinStamp', 'field_8eb4e427b80ac66d870fc0a5a0cc22ba'))) {
