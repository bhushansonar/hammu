 ...
 * @since 1.3.1
 */
class SKAPI_BOL_HammuDao extends OW_BaseDao {
    ...
    public function findValueExistOrNotByUserIds($userId) {
        if (empty($userId)) {
            return array();
        }
        // echo "userin hamuu->" . $userId;
        $userId = trim($userId);
        $example = new OW_Example();
        $example->andFieldEqual("userId", $userId);
        return $this->findListByExample($example);
    ...
    public function deleteUserSkapi($userId) {
        $sql = "DELETE FROM `" . $this->getTableName() . "` WHERE `userId` = ? OR `userId` = ?";
        $this->dbo->query($sql, array($userId, $userId));
    }

    public function deleteUserFavorite($userId) {
        $table = "ow_favorite";
        $sql = "DELETE FROM `" . $table . "` WHERE `user_id` = ? OR `escort_id` = ?";
        $this->dbo->query($sql, array($userId, $userId));
    }
