 ...
 * @since 1.6.0
 */
class SKAPI_CLASS_EventHandler {
    ...
    public function trackVisit(OW_Event $event) {
        ...
        if ($userId && $guestId && ($guestId != $userId) && !$isAdmin) {
            SKAPI_BOL_Service::getInstance()->trackVisit($userId, $guestId);
        }
    }
    ...
    public function onUserUnregister(OW_Event $event) {
        ...
        SKAPI_BOL_Service::getInstance()->deleteUserGuests($userId);
    }
    ...
    public function getList(OW_Event $event) {
        ...
        $users = SKAPI_BOL_Service::getInstance()->findGuestUsers($userId, $page, $limit);
        $guestsIdList = array();
        foreach ($users as $user) {
            $guestsIdList[] = $user->id;
        }

        $guests = SKAPI_BOL_Service::getInstance()->findGuestsByGuestIds($userId, $guestsIdList);
        $out = array();
    ...
    public function getNewCount(OW_Event $event) {
        ...
        $count = SKAPI_BOL_Service::getInstance()->findNewGuestsCount($userId);
    ...
    public function markViewed(OW_Event $event) {
        ...
        SKAPI_BOL_Service::getInstance()->setViewedStatusByGuestIds($userId, $guestIds);
    }
